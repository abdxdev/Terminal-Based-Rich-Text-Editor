...
void GapBuffer::insert(char c) {
	...
	// Only clear redo stack if this is not a redo operation
	if (!isRedoOperation) {
	...
	// Rest of the insert method remains the same...
...
void GapBuffer::deleteCharLeft() {
	if (gapStart > 0) {
		// Check if we're deleting part of an ANSI sequence
		if (isInAnsiSequence(gapStart - 1)) {
			// Delete the entire ANSI sequence
	...
	if (to == -1) {
		...
		for (int i = 0; i < buffer.size(); i++) {
			...
			if (!isInAnsiSequence(i)) {
				...
				cout << buffer[i]; // Always output ANSI sequences
			}
		}
	}
}
